...
class Lead(SellingController, CRMNote):
	...
	def update_lead_duplicates(self):
		lead_list = frappe.db.get_list("Lead", {'custom_primary_phone': self.custom_primary_phone}, 
			...
			"creation"])
		lead_list += frappe.db.get_list("Lead", {'email_id': self.email_id}, 
			['name', "company_name", "email_id", "custom_primary_phone", "source", "custom_source_details",
			"creation"])
		...
		self.custom_lead_duplicate_details = []
		for row in lead_details:
			self.append("custom_lead_duplicate_details",{
				"name1": row["company_name"],
				"lead_id": row["name"], 
				"email": row["email_id"],
				"phone_no": row["custom_primary_phone"],
				"source": row["source"],
				"source_detail": row["custom_source_details"] if "custom_source_details" in row and row["custom_source_details"] else None,
				"created_date_and_time": row["creation"]
			})
