...
def update_lead_duplicates(self):	
    ...
    if lead_details:	
        ...
        frappe.msgprint(f"Parent lead: {lead_doc.name}")
        ...
        frappe.msgprint(f"Attempting to save lead_doc with name: {lead_doc.name}")
        ...
        frappe.msgprint(f"Successfully saved lead_doc with name: {lead_doc.name}")
        
        frappe.flags.in_update_lead_duplicates = False        	        

        # Raise a validation exception to stop the save operation
        frappe.throw(f"This lead is a duplicate of: {lead_doc.name}.<br> Lead duplicates updated successfully.", title="Duplicate Lead")
